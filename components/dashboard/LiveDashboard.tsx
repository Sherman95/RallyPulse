...
import { getCurrentEvent, ActiveEvent } from '@/lib/itineraryHelper';
...
export default function LiveDashboard({ onStatusChange }: LiveDashboardProps) {
  ...
  useEffect(() => {
    ...
    // Opcional: Recalcular cada minuto
    ...
    <div className="w-full flex flex-col animate-in fade-in duration-500">
      ...
      {activeEvent && (
        <div className="max-w-5xl mx-auto w-full px-3 sm:px-4 mb-4">
          <div className={`flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 rounded-xl border ${activeEvent.isNow ? 'bg-rally-accent/10 border-rally-accent/30' : 'bg-rally-surface2 border-rally-border'}`}>
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs font-bold uppercase tracking-wider ${activeEvent.isNow ? 'text-rally-accent' : 'text-rally-muted'}`}>
                  {activeEvent.isNow ? 'Corriendo Ahora' : 'Próxima Actividad'}
                </span>
                <span className="text-[10px] text-rally-muted px-2 py-0.5 rounded-full bg-black/20 border border-white/5">
                  {activeEvent.startTime.toLocaleDateString('es-EC', { weekday: 'short', day: 'numeric', month: 'short' })}
                </span>
              </div>
              <div className="text-sm sm:text-base font-bold text-rally-txt mt-1">
                {activeEvent.etapa} <span className="text-rally-muted mx-1">•</span> {activeEvent.actividad}
              </div>
              {activeEvent.lugar && (
            ...
            <div className="mt-3 sm:mt-0 flex flex-col sm:items-end bg-black/20 sm:bg-transparent p-2 sm:p-0 rounded-lg sm:rounded-none">
              ...
              <span className="text-sm font-mono font-bold text-rally-txt">
                {activeEvent.startTime.toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit' })} 
                {activeEvent.endTime && ` - ${activeEvent.endTime.toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit' })}`}
              </span>
            </div>
          </div>
        </div>
