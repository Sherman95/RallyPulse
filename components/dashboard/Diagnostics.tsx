...
export default function Diagnostics({ results }: DiagnosticsProps) {
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="fixed bottom-4 right-4 bg-rally-surface border border-rally-surface text-rally-txt px-4 py-2 rounded-full text-xs shadow-xl hover:bg-rally-bg transition z-50 flex items-center gap-2 font-medium"
      >
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 max-h-[80vh] flex flex-col bg-rally-surface border border-rally-surface shadow-2xl rounded-xl z-50 overflow-hidden">
      
      {/* Header */}
      <div className="flex items-center justify-between bg-rally-bg px-4 py-3 border-b border-rally-surface">
        <h3 className="text-sm font-bold text-rally-txt flex items-center gap-2">
        ...
        <button onClick={() => setIsOpen(false)} className="text-rally-muted hover:text-rally-txt transition-colors">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-px bg-rally-surface border-b border-rally-surface">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-rally-bg">
        {results.map(r => {
          ...
          return (
            <div key={r.numero} className="flex items-center justify-between p-2 rounded-lg bg-rally-surface border border-rally-surface shadow-sm text-sm">
              <div className="flex items-center gap-3 overflow-hidden">
                <span className="font-mono text-rally-muted font-semibold w-8">#{r.numero}</span>
                <span className="text-rally-txt truncate font-medium">
